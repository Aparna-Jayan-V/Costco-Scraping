...
def extract_content_soup(url):
    driver.get(url)
    page_content = driver.page_source
    soup = BeautifulSoup(page_content, 'html.parser')
    return soup

def extract_content_dom(url):
    soup=extract_content_soup(url)
...
def product_links(soup):
    product_urls = []
    content = extract_content_soup(category_links(soup)[0])
...
def get_price(dom):
    try:
        product_price = dom.xpath('//*[@id="breakdown"]/div[1]/span[2]/text()')
...
def get_colour(dom):
    try:
        product_colour = dom.xpath('//*[@id="nav-pdp-tab-header-4"]/div/div[1]/div/div[3]/div[2]/text()')
...
def get_type_of_headphones(dom):
    try:
        product_type = dom.xpath('//*[@id="nav-pdp-tab-header-4"]/div/div[1]/div/div[6]/div[2]/text()')
...
for product in range(len(data)):
    ...
    product_content = extract_content_dom(product_url)
